...
class PinViewModel : ViewModel() {
    ...
    val passwordLiveData: LiveData<CharSequence> by lazy {
        _passwordLiveData
    }
    ...
    private val _messageLiveDate by lazy { MutableLiveData<String>() }
    ...
    fun done(){
       if(validPin()){
           _messageLiveDate.value = "설정한 비밀번호는 $password 입니다."
           reset()
       }
    }
    ...
    private fun validPin() : Boolean{
        ...
        if(Pattern.compile("(\\w)\\1\\1").matcher(password.toString()).find()){
            _messageLiveDate.value = "3자리 이상 같은 숫자는 사용하실 수 없습니다."
            reset()
            return false
        }
        ...
        password.reduce { before, after ->
            if(after - before == 1){
                count++
                if(count >= 2){
                    _messageLiveDate.value = "연속된 3자리 숫자는 사용하실 수 없습니다."
                    reset()
                    return false
                }
            }else{
