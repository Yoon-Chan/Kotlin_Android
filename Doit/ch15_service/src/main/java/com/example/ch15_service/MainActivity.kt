...
class MainActivity : AppCompatActivity() {
    ...
    //aidl...........
    var aidService : MyAidlInterface? = null
    ...
    val aidlConnection : ServiceConnection = object : ServiceConnection {
        override fun onServiceConnected(p0: ComponentName?, p1: IBinder?) {
            aidService = MyAidlInterface.Stub.asInterface(p1!!)
            aidService!!.start()
            binding.aidlProgress.max = aidService!!.maxDuration
            val backgroundScope = CoroutineScope(Dispatchers.Default + Job())
    ...
    private fun onCreateJobScheduler(){
        var jobScheduler : JobScheduler? = getSystemService(JOB_SCHEDULER_SERVICE)
            as JobScheduler
